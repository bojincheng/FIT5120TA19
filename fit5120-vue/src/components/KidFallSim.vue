<template>
    <div ref="containerRef" class="relative w-full h-screen overflow-hidden bg-black text-white font-sans">
      <a
        href="#"
        class="absolute top-4 left-4 flex items-center gap-2 px-4 py-2 bg-black/70 text-white rounded-full hover:bg-black/90 transition-colors z-[11]"
      >
        <ArrowLeft class="w-5 h-5" />
        Back
      </a>
  
      <!-- Use a fixed-ratio container to wrap the canvas -->
      <div class="absolute inset-0 flex items-center justify-center">
        <div ref="canvasContainerRef" class="relative w-full h-full max-w-[1200px] max-h-[600px]" style="aspect-ratio: 2/1;">
          <canvas ref="canvasRef" class="absolute top-0 left-0 w-full h-full"></canvas>
        </div>
      </div>
  
      <!-- Start Simulation button -->
      <div v-if="!isStarted" class="absolute inset-0 flex items-center justify-center z-[50]">
        <button
      ...
      <!-- Quiz popup - 增加 z-index 值 -->
      <div v-if="showQuiz" class="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-[100]">
      ...
      <!-- Feedback popup - 增加 z-index 值 -->
      <div v-if="showFeedback" class="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-[100]">
        <div class="bg-gray-900 rounded-lg max-w-lg p-6 relative max-h-[90vh] overflow-y-auto">
          ...
          <div v-if="selectedOption === 'A'" class="mb-4">
            <div class="bg-yellow-900/50 p-4 rounded-md mb-4">
              <p class="text-yellow-200">
                <span class="font-bold">Risky choice:</span> Jumping into the water without proper
                training is dangerous. Without proper training and equipment, rescuers frequently become victims.
              </p>
            </div>
            <p class="text-gray-300">
              Jumping in was a risky decision that put you in danger as well. A drowning person can easily pull a rescuer underwater in their panic. Always prioritize your own safety when attempting to help others.
            </p>
          </div>
  
          <div v-if="selectedOption === 'B'" class="mb-4">
            <div class="bg-yellow-900/50 p-4 rounded-md mb-4">
              <p class="text-yellow-200">
                <span class="font-bold">Important:</span> Calling emergency services is crucial, but in a
                drowning situation, every second counts. Someone needs to take immediate action while waiting for
                help.
              </p>
            </div>
            <p class="text-gray-300">
              The best approach is to call for help AND take immediate action to assist the child without
              endangering yourself.
            </p>
          </div>
  
          <div v-if="selectedOption === 'C'" class="mb-4">
            <div class="bg-green-900/50 p-4 rounded-md mb-4">
              <p class="text-green-200">
                <span class="font-bold">Good choice:</span> Shouting for help alerts others, including
                lifeguards who are trained for water rescue. This is often the safest first response while you look
                for a reaching aid.
              </p>
            </div>
            <p class="text-gray-300">
              After shouting for help, look for a reaching pole, life ring, or other flotation device to help the
              child without entering the water yourself.
            </p>
          </div>
  
          <div v-if="selectedOption === 'timeout'" class="mb-4">
            <div class="bg-red-900/50 p-4 rounded-md mb-4">
              <p class="text-red-200">
                <span class="font-bold">Time's up!</span> In an emergency situation, quick action is essential.
                Every second counts when someone is drowning.
              </p>
            </div>
            <p class="text-gray-300">
              Remember to stay calm but act quickly. Alert others and look for safe ways to help without endangering
              yourself.
            </p>
          </div>
  ...
  // State variables - removed unused variables showIndicators and isFullscreen
  ...
  const drawPerson = (ctx, x, y, baseSize, legSwing, armSwing, rotation, colors, inWater, falling) => {
    ...
    // Removed unused variable submergeLevel
  ...
  const drawWalkingPerson = (ctx, width, height, time, colors) => {
    ...
    // Swimming/struggling phase (0.8-1.0)
      ...
      // Removed unused struggleProgress variable
  ...
  const drawRescueAnimation = (ctx, width, height, time, colors, type) => {
    ...
    if (type === "A") {
      ...
      // Swimming toward child (0.3-0.6)
        ...
        // Swimming animation - removed unused variables legKick and armStroke
        //const swimCycle = time * 4;
      ...
      // Struggling and calling for help (0.6-1.0)
        ...
        // Struggling animation - removed unused struggleIntensity variable
      ...
      // Removed unused shoreY variable
      ...
      // Removed unused returningToShore variable
      ...
      // Lifeguard returns to shore with child (0.75-1.0)
        ...
        if (returnProgress > 0.8) {
          ...
          lifeguardY = height * 0.45 - exitProgress * (height * 0.45 - height * 0.29);
  ...
  onMounted(() => {
    ...
    // Don't auto-start animation, wait for user to click start button
    // startAnimation();
  });
  ...
  // Timer for quiz - fixed unused parameters
